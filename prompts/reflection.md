...
3. `mcp__bobe__get_trades` pair, from, to, status="completed" — closed positions of the window; each with a nested `orders` array (all orders: action open/add/close, comp_size/comp_amount/comp_price, status, created_at). Position fields: `realized_pnl_pct`, `realized_pnl_usd`, `reason`, `side`, `regime_at_entry`, `features_at_entry`, `opened_price`, `opened_amount`, `closed_price`, `closed_amount`.
...
- closed trades are exits by **take-profit** (reason="take-profit") — the only exit; count and characterize them;
