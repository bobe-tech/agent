...
**The strategy thresholds in force — ONLY the active `config`** (from `get_params_history`, the version with `is_active`) and the current strategy mechanics (entry — counter-trend pullback (buy the dip below close_H1) + CRSI, averaging by depth+CRSI, exit — take-profit). From nowhere else.
...
`get_missed_moves` searches for HOLD in the trend structure `close>sma20>sma50` (or its mirror down), which does NOT match the entry gate (counter-trend pullback dip-buy below close_H1 + CRSI + ADX). So treat its output as a very **rough hint** only ("price was moving, we stood still"), not as direct evidence about the dip-buy gate. It measures, over `horizon_hours` (default 4) after each trending HOLD, whether the price moved by ≥ `move_threshold_pct` (default 3%) in the trend direction; tighten/loosen these knobs to change how sensitive the detection is. If there are many trend HOLDs and it was mainly the ADX threshold that blocked (field `gate_blocks.adx_lo`) — that is an argument to recommend LOWER `adx_lo`. Note: **only the ADX gate is recorded in this output** (`gate_blocks.adx_lo`) — it does not track the pullback/CRSI conditions, so it cannot tell you which of those blocked an entry (this is a property of the output, not of the entry logic, which checks ADX + pullback + CRSI). Consecutive HOLDs of one trend are correlated — count by the number of separate moves, not ticks.
...
- **The strategy type is unchanged:** no-stop DCA (entry — counter-trend pullback dip-buy + CRSI, averaging down, exit only by take-profit). ONLY the thresholds in `config` are tuned; the mechanics are not changed.
